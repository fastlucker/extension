...
class ProviderController {
  ...
  ethRpc = (req, forceChainServerId?: string) => {
    ...
    // }
    // const currentAddress = preferenceService.getCurrentAccount()?.address.toLowerCase() || '0x'
    ...
    // const chain = Object.values(CHAINS).find((item) => item.serverId === chainServerId)!
    // if (RPCService.hasCustomRPC(chain.enum)) {
    //   const promise = RPCService.requestCustomRPC(chain.enum, method, params).then((result) => {

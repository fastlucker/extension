...
test.describe('swapAndBridgePage Smart Account', () => {
  ...
  test('should switch tokens during Swap & Bridge with a Smart Account', async () => {
    ...
    await pages.swapAndBridgePage.prepareSwapAndBridge(null, fromToken, toToken)
  ...
  test('should do MAX token "From" amount during Swap & Bridge with a Smart Account', async () => {
    ...
    await pages.swapAndBridgePage.verifySendMaxTokenAmount(dai)
  ...
  test('should find token that already exists within the "Receive" list during Swap & Bridge with a Smart Account', async () => {
    ...
    await pages.swapAndBridgePage.verifyDefaultReceiveToken(usdc, wallet)
  ...
  test('should "reject" (ie cancel) Swap & Bridge from the Pending Route component with a Smart Account', async () => {
    ...
    const wallet = tokens.wallet.base
    await pages.swapAndBridgePage.openSwapAndBridge()
    await pages.swapAndBridgePage.prepareSwapAndBridge(0.8, usdc, wallet)
  ...
  test('should "proceed" Swap & Bridge from the Pending Route component with a Smart Account', async () => {
    ...
    await test.step('assert no transaction on Activity tab', async () => {
      await pages.swapAndBridgePage.checkNoTransactionOnActivityTab()
    })

    await test.step('prepare swap and bridge transaction', async () => {
      await pages.swapAndBridgePage.openSwapAndBridge()
      await pages.swapAndBridgePage.prepareSwapAndBridge(0.005, usdc, wallet)
    })

    await test.step('proceed and sign the transaction', async () => {
      await pages.swapAndBridgePage.proceedTransaction()
    })

    await test.step('assert new transaction on Activity tab', async () => {
      await pages.swapAndBridgePage.checkSendTransactionOnActivityTab()
    })
  })
  ...
  test('should switch from token amount to USD value and vise-versa during Swap & Bridge with a Smart Account', async () => {
    ...
    await pages.swapAndBridgePage.switchUSDValueOnSwapAndBridge(usdce, 0.34)
    await pages.swapAndBridgePage.switchUSDValueOnSwapAndBridge(dai, 0.2)
    await pages.swapAndBridgePage.switchUSDValueOnSwapAndBridge(usdc, 0.02)
    await pages.swapAndBridgePage.switchUSDValueOnSwapAndBridge(xwallet, 1)
    await pages.swapAndBridgePage.switchUSDValueOnSwapAndBridge(dai, 0.51)
  ...
  test('should auto-refresh active route after 60s during Swap & Bridge with a Smart Account', async () => {
    ...
    await pages.swapAndBridgePage.prepareSwapAndBridge(0.009, usdc, wallet)
  ...
  test('should Bridge tokens with a Smart Account', async () => {
    ...
    await test.step('prepare bridge transaction', async () => {
      await pages.swapAndBridgePage.prepareBridgeTransaction(0.0063, usdc, usdcOpt)
    })

    await test.step('sign transaction', async () => {
      await pages.swapAndBridgePage.signTokens()
    })
  ...
  test('should batch Swap of ERC20 tokens and Native to ERC20 token with a Smart Account', async () => {
    ...
    await test.step('start monitoring requests', async () => {
      await pages.swapAndBridgePage.monitorRequests()
    })

    await test.step('add a transaction swapping WALLET for USDC to the batch', async () => {
      await pages.swapAndBridgePage.prepareSwapAndBridge(0.003, wallet, usdc)
      await pages.swapAndBridgePage.batchAction()
    })

    await test.step('add a transaction swapping USDC for WALLET to the existing batch and sign', async () => {
      await pages.swapAndBridgePage.prepareSwapAndBridge(0.002, usdc, wallet)
      await pages.swapAndBridgePage.batchActionWithSign()
    })

    await test.step('stop monitoring requests and expect no uncategorized requests to be made', async () => {
      const { uncategorized } = pages.swapAndBridgePage.getCategorizedRequests()

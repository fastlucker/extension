...
export async function sendNoRelayer({
  finalBundle,
  account,
  network,
  wallet,
  ...
  if (signer.quickAccManager)
    throw new Error('quickAccManager not supported in relayerless mode yet')
  ...
  const isDeployed = await provider.getCode(finalBundle.identity).then((code: any) => code !== '0x')
  let gasLimit
  let to
  let data
  if (isDeployed) {
    ...
    data = FactoryInterface.encodeFunctionData('deployAndExecute', [
      account.bytecode,
      account.salt,
      finalBundle.txns,
      finalBundle.signature
    ])
  ...
  try {
    ...
    if (!wallet.sendTransaction) {
      ...
      if (network.chainId !== chainId)
        throw new Error(`Connected to the wrong network: please switch to ${network.name}`)
      txId = (
    ...
    if (e.code === 'INSUFFICIENT_FUNDS')
      throw new Error(
        `Insufficient gas fees: you need to have ${network.nativeAssetSymbol} on ${signer.address}`
      )
    throw e
  }
}

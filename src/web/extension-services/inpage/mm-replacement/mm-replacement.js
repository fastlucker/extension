...
let shouldReplaceMM = false // this should be let (not const) because is modified in inpage.ts as well

let hasWeb3ConnectButtonInPage = false

function getMMOccurrences() {
  // Get all text content on the webpage
  const allText = document?.body?.textContent || document?.body?.innerText
  // Count the occurrences of the specified word
  const mmWordCount = (allText?.match(new RegExp('metamask', 'gi')) || [])?.length

  return mmWordCount || 0
}

function findConnectButtonInPage() {
    ...
    Array.from(node.childNodes).forEach((childNode) => {
      if (childNode.nodeType === Node.TEXT_NODE) {
        ...
        if (text.toLowerCase() === 'connect wallet' || text.toLowerCase() === 'connect') {
          const rect = childNode.parentElement.getBoundingClientRect()
          const isTopRight = rect.top <= 200 && rect.right <= window.innerWidth - 300
          if (isTopRight) {
            hasWeb3ConnectButtonInPage = true
          }
        }
      }
    })
      ...
      if (childNode.nodeType === Node.TEXT_NODE) {
        ...
        if (/^walletconnect$/i.test(text.trim())) {
          if (childNode.parentElement.tagName === 'BUTTON') {
            ...
            const buttonWithWalletConnect = childNode.parentElement.closest('button')
...
const replaceMMIfNeeded = () => {
  ...
  const observer = new MutationObserver((mutationsList) => {
    ...
    const numberOfTimesMMWordOccursInPage = getMMOccurrences()

    if (numberOfTimesMMWordOccursInPage > 1) {
      return
    }

    if (!hasWeb3ConnectButtonInPage) {
      findConnectButtonInPage()
    }

    timeoutId = setTimeout(() => {
      if (shouldReplaceMM || hasWeb3ConnectButtonInPage) {

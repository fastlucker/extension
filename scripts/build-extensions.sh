...
WEBKIT_ZIP="./build/ambire-extension-v${VERSION}-webkit.zip"
GECKO_ZIP="./build/ambire-extension-v${VERSION}-gecko.zip"
WEBKIT_SOURCEMAPS_ZIP="./build/ambire-extension-v${VERSION}-webkit-sourcemaps.zip"
GECKO_SOURCEMAPS_ZIP="./build/ambire-extension-v${VERSION}-gecko-sourcemaps.zip"

# Create zip files
echo "Creating zip files..."
if [ -d "$WEBKIT_BUILD_DIR" ]; then
  zip -r "$WEBKIT_ZIP" "$WEBKIT_BUILD_DIR"
...
if [ -d "$GECKO_BUILD_DIR" ]; then
  zip -r "$GECKO_ZIP" "$GECKO_BUILD_DIR"
...
if [ -d "$WEBKIT_SOURCEMAPS_BUILD_DIR" ]; then
  zip -r "$WEBKIT_SOURCEMAPS_ZIP" "$WEBKIT_SOURCEMAPS_BUILD_DIR"
...
if [ -d "$GECKO_SOURCEMAPS_BUILD_DIR" ]; then
  zip -r "$GECKO_SOURCEMAPS_ZIP" "$GECKO_SOURCEMAPS_BUILD_DIR"
